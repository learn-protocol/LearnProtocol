...
export async function Vote({
    ...
    const updateGlobalStats = () =>
        db
            .update(stats)
            .set({
                distributedUpvotePrize: sql`${stats.distributedUpvotePrize} + ${upvote.upvotePrize}`,
                distributedDownvoteCutoff: sql`${stats.distributedDownvoteCutoff} + ${upvote.downvoteCutoff}`,
            })
            .where(eq(stats.id, 1));
